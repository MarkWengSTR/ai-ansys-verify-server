...
def report_setting(ctx):
    ...
    for data_name, x_axis in ctx["data"]["report_list"].values():
...
def report_export(ctx):
    ...
    for report_name, data_with_x_axis in ctx["data"]["report_list"].items():
...
def remove_all_report(ctx):
    print("remove all report")
    ctx["data"]["report_moudule"].DeleteAllReports()
